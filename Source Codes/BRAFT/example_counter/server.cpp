...
public:
    ...
    void fetch_add(const FetchAddRequest* request,
        ...
        return _node->apply(task); // 分发给其他 follower，等多数 flolower 成功后调用 on_apply(自己实现的，继承的 StateMachine 中的 on_apply)
    }
...
friend class FetchAddClosure;
    ...
    // @braft::StateMachine iter --> 日志，具体运行内容。
    void on_apply(braft::Iterator& iter) {
        // A batch of tasks are committed, which must be processed through
        // |iter|
        for (; iter.valid(); iter.next()) { // 处理每一条日志。
            int64_t detal_value = 0;
            CounterResponse* response = NULL;
            // This guard helps invoke iter.done()->Run() asynchronously to
            // avoid that callback blocks the StateMachine.
            braft::AsyncClosureGuard closure_guard(iter.done());
            if (iter.done()) { // 如果有 done 就是 Leader。
                ...
                response = c->response(); // 只有 leader 有，他回复 client。
            ...
            const int64_t prev = _value.fetch_add(detal_value,
                                                  butil::memory_order_relaxed); // 从 engine（内存） 中取出 _value 和 client 中的 detal_value 相加。
            if (response) { // 有 response 说明是 leader
                response->set_success(true);
