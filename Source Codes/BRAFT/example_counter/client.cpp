...
DEFINE_bool(log_each_request, false, "Print log for each request"); // TODO DEFINE_bool 是什么意思？
...
bvar::LatencyRecorder g_latency_recorder("counter_client"); // TODO 这条语句是啥意思？

static void* sender(void* arg) {
    while (!brpc::IsAskedToQuit()) {
        braft::PeerId leader; // TODO 声明 leader。话说，这个不应该写到 while 循环外面？
        ...
        if (channel.Init(leader.addr, NULL) != 0) { // 如果 Channel 创建失败。
            LOG(ERROR) << "Fail to init channel to " << leader;
            bthread_usleep(FLAGS_timeout_ms * 1000L);
            continue;
        }
        example::CounterService_Stub stub(&channel); // 构造 Stub。
...
int main(int argc, char* argv[]) {
    ...
    if (!FLAGS_use_bthread) {
        for (int i = 0; i < FLAGS_thread_num; ++i) {
            if (pthread_create(&tids[i], NULL, sender, NULL) != 0) { // 在这里向 Server 通过 sender 发送 rpc。
